...
supervisor software to request M-mode to start/stop harts running in supervisor mode.
...
on the supervisor side need to verify if the cpu is really started or not.
...
*Returns* one of the following possible SBI error code thorugh sbiret.error.
...
Stop the calling hart and mark it unavailable to the supervisor mode software.
...
The return value from this function also depend on any concurrent `sbi_hart_start` or
`sbi_hart_stop` calls. That's why, it is not recommended to perform any critical
