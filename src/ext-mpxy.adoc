...
Support, SSE Event and MSI bits will be `0`.
...
| RW
| *MSI Control* +
MSI control for notification indication.
```
0 = Disable
1 = Enable
```
...
NOTE: If the message channel does not support or is not configured for a
notification indication mechanism, such as MSI or SSE, the supervisor software
can periodically invoke the poll operation `sbi_mpxy_get_notification_events`.
